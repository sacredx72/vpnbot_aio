...
<code>example.org https://dns.adguard.com/dns-query 94.140.14.14</code>",
            ...
            default:
                switch ($pac['transport']) {
                    ...
                    case 'AIO':
                        $links = [];
                        // WS
                        $links[] = "vless://{$c['inbounds'][0]['settings']['clients'][$i]['id']}@$domain:443"
                                    . "?flow="
                                    . "&path=%2Fws$hash"
                                    . "&security=tls"
                                    . "&sni=$domain"
                                    . "&fp=chrome"
                                    . "&type=ws"
                                    . "#{$c['inbounds'][0]['settings']['clients'][$i]['email']}-WS";
                        // Reality
                        $links[] = "vless://{$c['inbounds'][0]['settings']['clients'][$i]['id']}@$domain:443"
                                    . "?security=reality"
                                    . "&sni={$pac['reality']['domain']}"
                                    . "&fp=chrome&pbk={$pac['xray']}"
                                    . "&sid={$pac['reality']['shortId']}"
                                    . "&type=tcp"
                                    . "&flow=xtls-rprx-vision"
                                    . "#{$c['inbounds'][0]['settings']['clients'][$i]['email']}-Reality";
                        if ($pac['transport'] == 'AIO') {
                            ...
                            $links[] = "vless://{$c['inbounds'][0]['settings']['clients'][$i]['id']}@$domain:443"
                                        ...
                                        . "&fp=chrome"
                                        . "&alpn=h2"
                                        . "#{$c['inbounds'][0]['settings']['clients'][$i]['email']}-XHTTP";
                            // gRPC
                            $links[] = "vless://{$c['inbounds'][0]['settings']['clients'][$i]['id']}@$domain:443"
                                        . "?security=tls"
                                        . "&type=grpc"
                                        ...
                                        . "&sni=$domain"
                                        . "&fp=chrome"
                                        . "#{$c['inbounds'][0]['settings']['clients'][$i]['email']}-gRPC";
                            // Hysteria2
                            $links[] = "vless://{$c['inbounds'][0]['settings']['clients'][$i]['id']}@$domain:11443"
                                        . "?security=tls"
                                        . "&alpn=h3"
                                        . "&sni=$domain"
                                        . "&fp=chrome"
                                        . "&type=hysteria"
                                        . "#{$c['inbounds'][0]['settings']['clients'][$i]['email']}-Hy2";
                        }
                        return implode("\n\n", $links);
    ...
    public function xray($page = 0)
    {
        ...
        $data[] = [
            ...
            [
                'text'          => 'Both ' . ($p['transport'] == 'Both' ? ' ✅' : ''),
                'callback_data' => "/changeTransport Both",
            ],
            [
                'text'          => 'AIO ' . ($p['transport'] == 'AIO' ? ' ✅' : ''),
                'callback_data' => "/changeTransport AIO",
            ],
        ];
        ...
        switch ($_GET['t']) {
            case 's':
                ...
                switch ($pac['transport']) {
                    ...
                    case 'AIO':
                        ...
                        // VLESS WebSocket
                        $vlessWs = $baseOutbound;
                        $vlessWs['tag'] = 'VLESS-WS';
                        $vlessWs['settings']['vnext'][0]['users'][0] = [
                            'id'         => '~uid~',
                            'encryption' => 'none',
                            'flow'       => ''
                        ];
                        $vlessWs['streamSettings'] = [
                            "network"    => "ws",
                            "security"   => "tls",
                            "wsSettings" => ["path" => "/vlws?ed=2560"],
                            "tlsSettings" => ["allowInsecure" => false, "serverName" => '~domain~', "fingerprint" => $fingerprint, "alpn" => ["http/1.1"]]
                        ];
                        unset($vlessWs['mux']);
                        $outbounds[] = $vlessWs;

                        // VMess WebSocket
                        $vmessWs = $baseOutbound;
                        ...
                        $trojanH2['streamSettings'] = [
                            "network"    => "h2",
                            "security"   => "tls",
                            "h2Settings" => ["host" => ['~domain~']],
                            "tlsSettings" => ["allowInsecure" => false, "serverName" => '~domain~', "fingerprint" => $fingerprint, "alpn" => ["h2"]]
                        ];
                        unset($trojanH2['mux']);
                        $outbounds[] = $trojanH2;
                        ...
                        if ($pac['transport'] == 'AIO') {
                            // Add all protocol outbounds for AIO mode
                        }
                        ...
                        // Replace original
                        array_splice($c['outbounds'], $index, 1, $outbounds);
                        break;
            ...
            case 'si':
                ...
                switch ($pac['transport']) {
                    ...
                    case 'AIO':
                        ...
                        $ws = $baseOutbound;
                        $ws['tag'] = 'VLESS-WS';
                        unset($ws['tls']['reality']);
                        unset($ws['flow']);
                        $ws["transport"] = ["type" => "ws", "path" => "/ws$hash"];
                        $ws['tls']['server_name'] = '~domain~';
                        $outbounds[] = $ws;

                        // Reality
                        $reality = $baseOutbound;
                        $reality['tag'] = 'VLESS-Reality';
                        unset($reality["transport"]);
                        $reality['flow'] = 'xtls-rprx-vision';
                        $reality['tls']['reality']['public_key'] = '~public_key~';
                        $reality['tls']['server_name'] = '~server_name~';
                        $reality['tls']['reality']['short_id'] = '~short_id~';
                        $outbounds[] = $reality;
                        ...
                        if ($pac['transport'] == 'AIO') {
                            ...
                            $xhttp['tls'] = ["enabled" => true, "insecure" => false, "server_name" => "~domain~", "alpn" => ["h2"]];
                            $outbounds[] = $xhttp;
                        }
            ...
            case 'cl':
                ...
                switch ($pac['transport']) {
                    ...
                    case 'AIO':
                        $proxies = [];
                        $baseProxy = $c['proxies'][$index];

                        // WS
                        $ws = $baseProxy;
                        $ws['name'] = 'VLESS-WS';
                        unset($ws['flow']);
                        unset($ws['reality-opts']);
                        $ws["network"] = "ws";
                        $ws["ws-opts"]['path'] = "/ws$hash";
                        $ws["skip-cert-verify"] = false;
                        $ws['servername'] = '~domain~';
                        $proxies[] = $ws;

                        // Reality
                        $reality = $baseProxy;
                        $reality['name'] = 'VLESS-Reality';
                        unset($reality["ws-opts"]);
                        unset($reality["skip-cert-verify"]);
                        $reality["network"] = "tcp";
                        $reality['flow'] = 'xtls-rprx-vision';
                        ...
                        if ($pac['transport'] == 'AIO') {
                            // XHTTP
                            $xhttp = $baseProxy;
                            $xhttp['name'] = 'VLESS-XHTTP';
                            unset($xhttp['ws-opts']);
                            unset($xhttp['flow']);
                            unset($xhttp['reality-opts']);
                            $xhttp['network'] = 'xhttp';
                            $xhttp['client-fingerprint'] = 'chrome';
                            $xhttp['tls'] = true;
                            $xhttp['alpn'] = ['h2'];
                            $xhttp['servername'] = '~domain~';
                            $xhttp['skip-cert-verify'] = false;
                            $xhttp['xhttp-opts'] = [
                                'host' => '~domain~',
                                'path' => "/xhttp$hash",
                                'mode' => 'packet-up',
                        ...
                        // Replace original and add the group
                        array_splice($c['proxies'], $index, 1, $proxies);
                        $c['proxy-groups'][] = $group;
                        break;
    ...
    public function changeTransport($transport)
    {
        ...
        $clientsWs = $x['inbounds'][0]['settings']['clients'];
        foreach ($clientsWs as $k => $v) {
            unset($clientsWs[$k]['flow']);
        }
        $clientsReality = $x['inbounds'][0]['settings']['clients'];
        ...
        switch ($transport) {
            ...
            case 'AIO':
                // All-in-One fallbacks architecture by XTLS/Xray-examples
                // Single port 443 with Nginx frontend and fallback chain
                
                // Base inbound - Nginx fallback on port 443
                $nginxFallback = [
                    "port"     => 443,
                    "protocol" => "vless",
                    "settings" => [
                    ...
                    "streamSettings" => [
                        ...
                        "tlsSettings" => [
                            ...
                            "alpn" => ["http/1.1", "h2"],
                            "minVersion" => "1.2",
                            "cipherSuites" => "TLS_ECDHE_ECDSA_WITH_CHACHA20_POLY1305_SHA256:TLS_ECDHE_RSA_WITH_CHACHA20_POLY1305_SHA256:TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256:TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256"
                        ]
                    ],
                    "tag" => "vless_tls_fallback"
                ];
                ...
                // Shadowsocks WebSocket inbound (Unix socket)
                $ssWsInbound = [
                    "listen"   => "@ss-ws",
                ...
                // H2 backend for Trojan/H2 (Unix socket)
                $h2Backend = [
                    "listen"   => "@h2-backend",
                    "protocol" => "trojan",
                    "settings" => [
                ...
                // Default fallback to Nginx on port 8080
                $defaultFallback = [
                    "listen"   => "@default-fallback",
                    "protocol" => "dokodemo-door",
                    "settings" => [
                        "address" => "127.0.0.1",
                        "port"    => 8080
                ...
                // Reality inbound on separate port
                $realityInbound = [
                    "port"     => 33443,
                    "protocol" => "vless",
                    "settings" => [
                        "clients"    => $clientsReality,
                        "decryption" => "none",
                    ],
                    "sniffing" => $baseInbound['sniffing'],
                    "streamSettings" => [
        ...
        $this->setUpstreamDomain(in_array($transport, ['Reality', 'Both', 'AIO'], true) ? ($p['reality']['domain'] ?: 't') : 't');
        $this->setUpstreamRealityPort(in_array($transport, ['Both', 'AIO'], true) ? 33443 : 443);
        $this->setPacConf($p);
        $this->restartXray($x);
        $this->xray();
    }
